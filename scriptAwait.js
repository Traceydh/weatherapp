...
function getUserLocation() {
    //callback function that takes geolocationPosition object as input parameter 
    const successCallback = (position) => {
        let lat = position.coords.latitude;
        let lon = position.coords.longitude;
        //Get today's weather object based on user's coordinate location 
        fetchUserLocationWeatherData(lat, lon);
    }
...
async function fetchWeeklyWeather(city) {
    try {
        ...
        for (let i = 8; i < 40; i += 8) {
              writeWeeklyWeather(responseData.list[i]);
        }
        writeWeeklyWeather(responseData.list[39]);
    } catch (error) {
        console.log(error)
    }
}

function writeWeatherData(response) {
    //city 
